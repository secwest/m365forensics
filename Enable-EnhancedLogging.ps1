...
$allRunningServices = Get-Service | Where-Object { $_.Status -eq "Running" -and $_.StartType -eq "Automatic" }
$suspiciousServices = @()
...
# Check for suspicious prefetch activity during off-hours
if ($allPrefetch) {
    $offHoursPrefetch = $allPrefetch | Where-Object {
        $hour = $_.LastRunTime.Hour
        $hour -in @(0..6, 23) -and $_.LastRunTime -gt $StartTime
    }
...
# Check for activity during unusual hours
$suspiciousTimes = Get-WinEvent -FilterHashtable @{
...
# Check elevation service activity
$elevationEvents = Get-WinEvent -FilterHashtable @{
...
$bootEvents = Get-WinEvent -FilterHashtable @{
    LogName = 'System'
    Id = 12, 13, 6005, 6006, 6008, 6009, 1074  # Boot related event IDs
    StartTime = $StartTime
    EndTime = $EndTime
...
$recentTasks = Get-ScheduledTask | Where-Object { 
    ...
    Name='LastRunTime'
    Expression={($_ | Get-ScheduledTaskInfo).LastRunTime}
}, @{
    Name='NextRunTime'
    Expression={($_ | Get-ScheduledTaskInfo).NextRunTime}
}

if ($recentTasks) {
    $out = "Recently Active Scheduled Tasks:`r`n"
    $out += $recentTasks | Format-Table -AutoSize | Out-String
    ...
    if ($suspiciousTasks) {
        ...
        $out += "`r`nSUSPICIOUS TASKS:`r`n"
        $out += $suspiciousTasks | Format-Table -AutoSize | Out-String
    }
} else {
...
if (-not $foundElevation) {
    ...
    # Search for it elsewhere
    Write-Host "Searching system for elevation_service.exe..." -ForegroundColor Yellow
    $searchResults = Get-ChildItem -Path C:\ -Filter "elevation_service.exe" -Recurse -ErrorAction SilentlyContinue -Force |
...
$suspiciousServices = Get-Service | Where-Object {
    ...
    ($_.DisplayName -match '(elevation|update|installer|admin)' -or
     $_.Name -match '^[a-z]{6,8}$' -or  # Random letter services
     $_.Name -match '^\d{4,}')           # Numeric services
} | Select-Object Name, DisplayName, @{
    Name='PathName'
    Expression={
        $serviceWmi = Get-WmiObject -Class Win32_Service -Filter "Name='$($_.Name)'" -ErrorAction SilentlyContinue
        $serviceWmi.PathName
    }
}
...
if ($suspiciousServices) {
    $out = "Potentially Suspicious Running Services:`r`n"
    $out += $suspiciousServices | Format-Table -Wrap -AutoSize | Out-String
    ...
    if ($tempServices) {
        Write-Alert "Found services running from temporary/user locations!"
        $out += "`r`nHIGH RISK - Services from temp/user folders:`r`n"
        $out += $tempServices | Format-Table -Wrap -AutoSize | Out-String
    }
} else {
    $out = "No obviously suspicious services found.`r`n"
}
...
try {
  $tasks = Get-ScheduledTask -TaskPath '\Microsoft\Windows\Installer\' -ErrorAction Stop
  $out   = $tasks |
           Select TaskName, LastRunTime, NextRunTime |
           Format-Table -AutoSize |
           Out-String
} catch {
  $out = "No Installer tasks found or access denied.`r`n"
...
if ($audits) {
  $out = $audits |
    Where-Object Message -match 'msiexec\.exe' |
...
if ($allPrefetch | Where-Object { $_.ProcessName -match 'elevation_service' }) {
    $summary += "[!] ELEVATION_SERVICE.EXE activity detected`r`n"
    $indicators++
}
...
Start-Process notepad $OutputPath) {
    ...
    # Tasks that ran during suspicious hours
    $suspiciousTimeTasks = $last24HoursTasks | Where-Object {
        $hour = $_.Info.LastRunTime.Hour
        $hour -in @(0..6, 23)
    }
...
# Check for elevation service in last 24 hours
if ($last24HoursPrefetch | Where-Object { $_.ProcessName -match 'elevation_service' }) {
    $elevationCount = ($last24HoursPrefetch | Where-Object { $_.ProcessName -match 'elevation_service' }).Count
    $summary += "[!] ELEVATION_SERVICE.EXE activity detected - $elevationCount executions in 24 hours`r`n"
    $indicators++
}
...
# Check for suspicious prefetch activity
if ($suspiciousTimePrefetch -and $suspiciousTimePrefetch.Count -gt 10) {
    $summary += "[!] Suspicious off-hours program execution: $($suspiciousTimePrefetch.Count) programs`r`n"
    $indicators++
}

# Check for rapid execution patterns
if ($rapidExecutions -and $rapidExecutions.Count -gt 0) {
    $summary += "[!] Rapid execution patterns detected: $($rapidExecutions.Count) bursts`r`n"
    $indicators++
}
...
Get-WinEvent -FilterHashtable @{
    LogName = 'Security'
    Id = 4688, 4672, 4673  # Process creation and privilege use
    StartTime = $StartTime
    EndTime = $EndTime
} -MaxEvents 2000 -ErrorAction SilentlyContinue | ForEach-Object {
    $window = [math]::Floor($_.TimeCreated.Minute / 5) * 5
    $key = "$($_.TimeCreated.ToString('yyyy-MM-dd HH')):$($window.ToString('00'))"
    
    if (-not $correlatedEvents.ContainsKey($key)) {
        $correlatedEvents[$key] = @()
    }
    $correlatedEvents[$key] += $_
}
    ...
    if ($suspiciousTimeTasks) {
        ...
        $out += $suspiciousTimeTasks | ForEach-Object {
            [PSCustomObject]@{
                ...
                'Full Path' = $_.FullPath
                'Execute' = if ($_.Task | Get-ScheduledTask) { 
                    ($_.Task | Get-ScheduledTask).Actions.Execute 
                } else { 'Unknown' }
            }
        } | Sort-Object Time | Format-Table -Wrap | Out-String
        Write-Host $out
        Write-Output $out
    }
...
if ($recentTasks) {
    ...
    $out += $taskSummary | Format-Table -Wrap | Out-String
    ...
    if ($suspiciousTasks) {
        ...
        foreach ($task in $suspiciousTasks) {
            ...
            $out += "`r`nSuspicious Task: $($task.TaskPath)$($task.TaskName)`r`n"
            if ($taskDetail.Actions) {
                $out += "Executes: $($taskDetail.Actions.Execute) $($taskDetail.Actions.Arguments)`r`n"
            }
        }
    }
} else {
