...
.DESCRIPTION
  ...
  - Process Creation Monitoring  #requires -RunAsAdministrator
<#
.SYNOPSIS
...
if ($suspiciousTimePrefetch) {
    ...
    $out = "`r`nPrograms Run During Off-Hours (11PM-6AM):`r`n"
...
Write-Host "Analyzing activity during off-hours (11PM-6AM)..." -ForegroundColor Yellow
...
if ($suspiciousTimes) {
    Write-Alert "Found $($suspiciousTimes.Count) events during off-hours"
...
2. INVESTIGATION PRIORITIES:
   - Review activities during off-hours (11PM-6AM) for your environment
...
try {
    ...
    if ($msi) {
        ...
        $suspiciousMsi = $msi | Where-Object {
            $hour = $_.TimeCreated.Hour
            $hour -in @(0..6, 23) -or
            $_.Message -match "(temp|appdata|users\\[^\\]+\\appdata|programdata)"
        }
...
# 6) Elevation Service Investigation
# ------------------------------------------------------------------------
Write-Section "Elevation Service Investigation" "Red"
...
if ($recentExes) {
    Write-Alert "Found $($recentExes.Count) recently modified executables in suspicious locations!"
    $out = "`r`nRecent Executables in Suspicious Locations:`r`n"
    $out += $recentExes | 
        Select-Object FullName, Length, LastWriteTime, CreationTime |
        Format-Table -AutoSize | 
        Out-String
...
# 9) Time-Based Analysis
# ------------------------------------------------------------------------
Write-Section "Time-Based Analysis" "Red"

# Define suspicious hours
$suspiciousHours = @(0..6) + @(23)

# Check for activity during unusual hours
Write-Host "Analyzing activity during suspicious hours (11PM-6AM)..." -ForegroundColor Yellow
$suspiciousTimes = @()
$logNames = @("Security", "Application", "System")
...
if ($suspiciousTimes) {
    Write-Alert "Found $($suspiciousTimes.Count) events during suspicious hours!"
    $grouped = $suspiciousTimes | 
        Group-Object { $_.TimeCreated.ToString("yyyy-MM-dd HH:00") } |
        Sort-Object Name
    
    $out = "`r`nEvents by hour during suspicious times:`r`n"
    foreach ($group in $grouped) {
        $out += "$($group.Name): $($group.Count) events`r`n"
...
# Correlation Analysis
Write-Host "`nPerforming Suspicious Pattern Correlation..." -ForegroundColor Yellow
try {
    $correlatedEvents = @{}
...
2. INVESTIGATION PRIORITIES:
   - Focus on activities during off-hours (11PM-6AM)
   - Investigate rapid execution patterns (potential automation)
   - Review all MSI installations you did not initiate
   - Check network connections from suspicious processes
   - Verify all non-Microsoft signed executables
...
6. INCIDENT RESPONSE:
   - If high-risk indicators found, consider isolating the system
   - Preserve evidence (memory dump, event logs, prefetch files)
   - Document all findings with timestamps
   - Check other systems for similar indicators
   - Consider engaging security professionals for forensic analysis"
...
if ($indicators -eq 0) {
    ...
    $summary += "CRITICAL - System compromise likely!`r`n"
    Write-Host $summary -ForegroundColor Red
}
