...
function RegSet {
    ...
    try {
        ...
        Log 'FAIL' "Failed to set registry value: $k\$n - $err"
    }
}
...
function IsZip {
    ...
    try { 
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Error checking if file is ZIP: $err"
        return $false 
    }
}
...
function EnsurePwsh7 {
    ...
    if(-not(Test-Path $exe)) {
        ...
        try {
            ...
            $err = $_.Exception.Message
            Log 'WARN' "Failed to install PowerShell 7: $err"
        }
    }
    ...
    while(-not $success -and $retryCount -lt $maxRetries) {
        try {
            ...
            if($retryCount -ge $maxRetries) {
                $err = $_.Exception.Message
                Log 'FAIL' "Failed to download after $maxRetries attempts: $err"
                return $false
            }
    ...
    try {
        ...
        if(-not $tpm.TpmReady) {
            ...
            try {
                ...
                $err = $_.Exception.Message
                Log 'FAIL' "Error initializing TPM: $err"
                return $false
            }
        }
        ...
        $err = $_.Exception.Message
        Log 'FAIL' "Error checking TPM: $err"
        return $false
    }
}
...
function VerifyHardening {
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Could not verify BitLocker status: $err"
    }
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Could not verify TPM status: $err"
    }
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Could not verify Device Guard status: $err"
    }
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Could not verify Defender status: $err"
    }
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Could not verify firewall rules: $err"
    }
}
...
if(-not $skip) {
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "PowerShell extraction failed, trying tar.exe fallback: $err"
        try {
            ...
            $err = $_.Exception.Message
            Log 'FAIL' "Both extraction methods failed: $err"
            $skip = $true
        }
    }
...
if(-not(Get-Module PSWindowsUpdate -ListAvailable)) {
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to install PSWindowsUpdate: $err"
    }
}
...
if(Get-Module PSWindowsUpdate -ListAvailable) {
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Windows Update failed: $err"
    }
    ...
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Defender signature update failed: $err"
    }
} else {
...
try {
    ...
    if($blvStatus.ProtectionStatus -ne 'ProtectionOn') {
        ...
        if(CheckTpm) {
            ...
            try {
                ...
                $err = $_.Exception.Message
                Log 'FAIL' "Failed to enable BitLocker: $err"
            }
        } else {
    ...
    $err = $_.Exception.Message
    Log 'FAIL' "Error checking BitLocker status: $err"
}
...
try {
    ...
    $err = $_.Exception.Message
    Log 'WARN' "Error disabling SMB1: $err"
}
    ...
    foreach($r in @('Server', 'Client')) {
        ...
        if(-not(Test-Path $path)) {
            try {
                ...
                $err = $_.Exception.Message
                Log 'WARN' "Failed to create registry path $path - $err"
            }
        }
    }
}
...
try {
    ...
    $err = $_.Exception.Message
    Log 'WARN' "Failed to set hypervisor launch type: $err"
}
...
try {
    ...
    $err = $_.Exception.Message
    Log 'WARN' "Failed to configure Defender settings: $err"
}
...
foreach($rule in $asrRules) {
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to enable ASR rule $($rule.Name): $err"
    }
}
...
foreach($a in @('Word', 'Excel', 'PowerPoint')) {
    ...
    if(-not(Test-Path $path)) {
        try {
            ...
            $err = $_.Exception.Message
            Log 'WARN' "Failed to create Office policy path $path - $err"
        }
    }
}
...
if(-not(Test-Path "$off\Common\Security")) {
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to create Office common security policy path: $err"
    }
}
...
if(-not(Test-Path "$off\Common\COM Compatibility")) {
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to create Office COM compatibility policy path: $err"
    }
}
...
try {
    ...
    $err = $_.Exception.Message
    Log 'WARN' "Failed to set PowerShell execution policy: $err"
}
...
foreach($path in @("$ps\ScriptBlockLogging", "$ps\ModuleLogging", "$ps\Transcription")) {
    if(-not(Test-Path $path)) {
        try {
            ...
            $err = $_.Exception.Message
            Log 'WARN' "Failed to create PowerShell policy path $path - $err"
        }
    }
}
...
if(-not(Test-Path "$ps\ModuleLogging\ModuleNames")) {
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to create PowerShell module names policy path: $err"
    }
}
...
if(-not(Test-Path 'C:\PowerShellTranscripts')) {
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to create PowerShell transcripts directory: $err"
    }
}
...
foreach($feature in $optionalFeatures) {
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to disable optional feature $feature - $err"
    }
}
...
foreach($category in $auditCategories) {
    try {
        ...
        $err = $_.Exception.Message
        Log 'WARN' "Failed to set audit policy for $category: $err"
    }
}
...
if($anydeskInstalled) {
    ...
    if(-not(Get-NetFirewallRule -DisplayName 'Hardening - AnyDesk TCP 7070' -ErrorAction SilentlyContinue)) {
        try {
            ...
            $err = $_.Exception.Message
            Log 'WARN' "Failed to create AnyDesk firewall rules: $err"
        }
    } else {
