<# =====================================================================
 Hardening-Win11Pro.ps1                                           (v2025-05-18)
 ...
 ❶ SELF-UPGRADE
     • If the script starts in Windows PowerShell 5.1 (x86 or x64) it
         – Silently installs **64-bit PowerShell 7**
         – Relaunches itself **in the same console**
             · from file   → re-executes that file
             · from stdin  → writes itself to %TEMP%, runs, then deletes
     • All later work therefore occurs under pwsh 7 x64.
 ...
 ❷ HARDENING STEPS
     ...
     5  Apply Defender baseline + *five* critical ASR rules
 ...
 ❸ RUNTIME LOGGING
     • Every command and its output are captured by **Start-Transcript** in
       `C:\HardeningLogs\Win11-Harden_<yyyyMMdd_HHmmss>.log`
     • Console still shows concise “[ * ] / [ ! ]” status messages.

 OPERATOR CHECKLIST
     ▸ Run in an **elevated** console (Administrator)
 ...
 NOTES
     • All registry + Defender settings are idempotent; you can re-run safely.
     • Windows Update can be disabled by commenting the two PSWindowsUpdate
       lines if you prefer to patch interactively later.
...
if (-not (Test-Path $logRoot)) { New-Item $logRoot -ItemType Directory | Out-Null }
$logFile = Join-Path $logRoot ("Win11-Harden_{0:yyyyMMdd_HHmmss}.log" -f (Get-Date))
Start-Transcript -LiteralPath $logFile -Force | Out-Null
Log-Info "Transcript started: $logFile"
...
Log-Info 'Applying cumulative updates (this may take several minutes) …'
...
Log-Warn 'Recovery keys copied to C:\RecoveryKeys — move them offline.'
...
# 8 ▸ Remove optional features (ignore missing)
